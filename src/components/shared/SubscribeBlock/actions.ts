    ...
    try {
        ...
        if (contact) {
            ...
            try {
                ...
                if (deletedCount === 0) {
                    console.error('Failed to remove user from database in "subscribe" action (email sending failed case 1)', error);
                }
        ...
        console.error('Failed to connect to MongoDB in "subscribe" action', error);
