...
const GET = async (request: NextRequest, context: RouteContext<'/api/data/voices/[bucketName]/[[...slug]]'>) => {
    const { bucketName, slug } = await context.params;
    const [lng, version] = slug!;

    try {
        const bucket = storage.bucket(bucketName);
        const prefix = version ? `voices/${lng}/${version}/` : `voices/${lng}/`;
        ...
        Sentry.captureException(error, {
            extra: {
                message: 'Error fetching files from Google Cloud Storage',
                bucketName
            }
        });
    }
