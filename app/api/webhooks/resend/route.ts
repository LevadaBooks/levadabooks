...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        Sentry.captureException(error, {
            extra: {
                message: 'Verify webhook error in "resend" webhook'
            }
        });
    ...
    if (['email.bounced', 'email.complained'].includes(event.type)) {
        try {
            ...
            try {
                ...
                Sentry.captureException(error, {
                    extra: {
                        message: 'Remove subsccribers error in "resend" webhook'
                    }
                });
            }
        } catch (error) {
            Sentry.captureException(error, {
                extra: {
                    message: 'Failed to connect to MongoDB in "resend" webhook'
                }
            });
        } finally {
            await client.close();
